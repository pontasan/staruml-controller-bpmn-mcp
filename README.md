# StarUML Controller MCP Server — BPMN Diagram
...
<p align="center">
  <img src="images/architecture.svg" alt="Architecture: Claude ↔ MCP (stdio) ↔ staruml-controller-bpmn-mcp ↔ HTTP REST API ↔ StarUML" width="800">
</p>
...
```bash
git clone https://github.com/pontasan/staruml-controller-bpmn-mcp.git
cd staruml-controller-bpmn-mcp
npm install && npm run build
```
...
5. All BPMN Diagram tools become available via MCP
...
```json
{
  "mcpServers": {
    "staruml-bpmn": {
      "command": "node",
      "args": ["/absolute/path/to/staruml-controller-bpmn-mcp/dist/index.js"],
...
```bash
claude mcp add staruml-bpmn -e STARUML_PASSWORD=<password> -- node /absolute/path/to/staruml-controller-bpmn-mcp/dist/index.js
```
...
The core of this server — full CRUD for every BPMN Diagram resource.

| Resource | Tools |
|---|---|
| **Diagrams** | `bpmn_list_diagrams`, `bpmn_create_diagram`, `bpmn_get_diagram`, `bpmn_update_diagram`, `bpmn_delete_diagram` |
| **Participants** | `bpmn_list_participants`, `bpmn_create_participant`, `bpmn_get_participant`, `bpmn_update_participant`, `bpmn_delete_participant` |
| — Lanes | `bpmn_list_participant_lanes`, `bpmn_create_participant_lane` |
| **Tasks** | `bpmn_list_tasks`, `bpmn_create_task`, `bpmn_get_task`, `bpmn_update_task`, `bpmn_delete_task` |
| **Sub Processes** | `bpmn_list_sub_processes`, `bpmn_create_sub_process`, `bpmn_get_sub_process`, `bpmn_update_sub_process`, `bpmn_delete_sub_process` |
| **Events** | `bpmn_list_events`, `bpmn_create_event`, `bpmn_get_event`, `bpmn_update_event`, `bpmn_delete_event` |
| — Event Definitions | `bpmn_list_event_event_definitions`, `bpmn_create_event_event_definition` |
| **Gateways** | `bpmn_list_gateways`, `bpmn_create_gateway`, `bpmn_get_gateway`, `bpmn_update_gateway`, `bpmn_delete_gateway` |
| **Data Objects** | `bpmn_list_data_objects`, `bpmn_create_data_object`, `bpmn_get_data_object`, `bpmn_update_data_object`, `bpmn_delete_data_object` |
| **Conversations** | `bpmn_list_conversations`, `bpmn_create_conversation`, `bpmn_get_conversation`, `bpmn_update_conversation`, `bpmn_delete_conversation` |
| **Choreographies** | `bpmn_list_choreographies`, `bpmn_create_choreography`, `bpmn_get_choreography`, `bpmn_update_choreography`, `bpmn_delete_choreography` |
| **Annotations** | `bpmn_list_annotations`, `bpmn_create_annotation`, `bpmn_get_annotation`, `bpmn_update_annotation`, `bpmn_delete_annotation` |
| **Sequence Flows** | `bpmn_list_sequence_flows`, `bpmn_create_sequence_flow`, `bpmn_get_sequence_flow`, `bpmn_update_sequence_flow`, `bpmn_delete_sequence_flow` |
| **Message Flows** | `bpmn_list_message_flows`, `bpmn_create_message_flow`, `bpmn_get_message_flow`, `bpmn_update_message_flow`, `bpmn_delete_message_flow` |
| **Associations** | `bpmn_list_associations`, `bpmn_create_association`, `bpmn_get_association`, `bpmn_update_association`, `bpmn_delete_association` |
| **Data Associations** | `bpmn_list_data_associations`, `bpmn_create_data_association`, `bpmn_get_data_association`, `bpmn_update_data_association`, `bpmn_delete_data_association` |
| **Message Links** | `bpmn_list_message_links`, `bpmn_create_message_link`, `bpmn_get_message_link`, `bpmn_update_message_link`, `bpmn_delete_message_link` |
| **Conversation Links** | `bpmn_list_conversation_links`, `bpmn_create_conversation_link`, `bpmn_get_conversation_link`, `bpmn_update_conversation_link`, `bpmn_delete_conversation_link` |
